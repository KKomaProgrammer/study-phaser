...
function slugOf(title) {
  return title.replace(/\s*\/\s*/g, '_').replace(/\s+/g, '_').replace(/[()]/g, '');
}
function norm(value = '') {
  return String(value).toLowerCase().replace(/[_()./]/g, ' ').replace(/\s+/g, ' ').trim();
}
function make(line, order) {
  const [title, category, tagsText] = line.split('|');
  const tags = tagsText.split(' ');
  const slug = slugOf(title);
  const summary = `${title} 문법은 ${category} 영역에서 자주 쓰는 Phaser 핵심 문법입니다. 이름, 옵션, 호출 위치를 함께 이해하면 실제 프로젝트 구조에 바로 적용할 수 있습니다.`;
  const syntax = title.includes('new Phaser.Game') ? 'new Phaser.Game(config);' : title.includes('preload') ? 'preload(){}\ncreate(){}\nupdate(time, delta){}' : `${title};`;
  const sections = [
    { heading: '무엇인가', paragraphs: [summary, '문법 이름의 앞부분은 어느 Phaser 시스템을 쓰는지 알려줍니다. 예를 들어 this.add는 화면 오브젝트 생성, this.input은 입력, this.time은 시간 제어입니다.'] },
    { heading: '기본 문법', code: syntax },
    { heading: '사용 순서', list: ['호출 위치를 먼저 정합니다.', '필요한 key, 좌표, option 값을 작은 예제로 확인합니다.', '동작이 확인되면 Scene 구조 안에 넣습니다.'] },
    { heading: '실전 포인트', list: ['문서 이름이 검색 우선순위에서 가장 높습니다.', '옵션 객체는 여러 줄로 쓰면 관리하기 쉽습니다.', '반복 등록 코드는 update가 아니라 create에 두는 것이 안전합니다.'] }
  ];
  const code = `const name = ${JSON.stringify(title)};\nconst syntax = ${JSON.stringify(syntax)};\nconsole.log(name);\nconsole.log(syntax);`;
  return { slug, title, category, summary, tags, sections, code, order, searchText: norm([title, category, tagsText, summary, syntax].join(' ')), titleText: norm(`${title} ${slug}`) };
}
...
function findDoc(slug) {
  const wanted = decodeURIComponent(String(slug || '')).replace(/^\/+|\/+$/g, '');
  return DOCS.find(d => d.slug === wanted) || DOCS.find(d => norm(d.title) === norm(wanted));
}
function related(slug) {
  const cur = findDoc(slug);
  if (!cur) return [];
  return DOCS.filter(d => d.slug !== cur.slug)
    .map(d => ({ d, score: (d.category === cur.category ? 20 : 0) + d.tags.filter(t => cur.tags.includes(t)).length * 8 - Math.abs(d.order - cur.order) * 0.01 }))
...
export async function onRequestGet({ request }) {
  ...
  if (slug) {
    ...
    return json({ doc: doc || null, related: doc ? related(doc.slug) : [] }, doc ? 200 : 404);
  }
  return json({ docs: DOCS.map(compact) });
}
