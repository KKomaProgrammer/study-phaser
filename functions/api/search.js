...
const DOCS=ROWS.trim().split('\n').map((line,order)=>{const[title,key,tagsText,pop]=line.split('|');const meta=CATS[key]||[key,[]],category=meta[0],aliases=meta[1],tags=tagsText.split(' '),slug=slugOf(title);const summary=title==='기본 HTML'?'Phaser를 실행할 기본 HTML 구조와 CDN, 루트 div, 스크립트 배치 방법입니다.':`${title} 문법은 Phaser의 ${category} 영역에서 자주 쓰입니다.`;return{title,categoryKey:key,category,aliases,tags,popular:pop==='1',slug,order,summary,titleText:norm(`${title} ${slug}`),searchText:norm([title,slug,key,category,aliases.join(' '),tagsText,summary].join(' '))}});
...
const VOCAB=[],WORD_SET=new Set();for(const d of DOCS){for(const w of norm([d.title,d.slug,d.category,d.categoryKey,d.aliases.join(' '),d.tags.join(' ')].join(' ')).split(' ')){if(w.length>=2&&!WORD_SET.has(w)){WORD_SET.add(w);VOCAB.push(w)}}}
...
function run(q,catParam){const p=parse(q,catParam),fix=correction(p),terms=p.terms.map(bestWord),keys=p.cats.map(c=>c.key),candidates=DOCS.filter(d=>!keys.length||keys.includes(d.categoryKey)),ranked=candidates.map(d=>({d,score:score(d,terms)})).filter(x=>x.score>0||(!terms.length&&keys.length)).sort((a,b)=>b.score-a.score||a.d.order-b.d.order||a.d.title.localeCompare(b.d.title,'ko'));const suggested=[];if(fix)suggested.push({type:'correction',text:fix.suggested,label:fix.message});for(const x of ranked.slice(0,7))suggested.push({type:'doc',text:x.d.title,label:x.d.title,slug:x.d.slug});for(const c of CATEGORIES){const ct=norm(`${c.name} ${c.key} ${c.aliases.join(' ')}`);if(!terms.length||terms.some(t=>ct.includes(t)))suggested.push({type:'category',text:'#'+c.name,label:'#'+c.name})}const seen=new Set(),suggestedQueries=[];for(const x of suggested){const k=norm(x.text);if(!seen.has(k)){seen.add(k);suggestedQueries.push(x)}if(suggestedQueries.length>=7)break}return{query:q,activeCategories:p.cats.map(c=>c.name),correction:fix,results:ranked.slice(0,18).map(x=>({...compact(x.d),snippet:x.d.summary,score:Math.round(x.score)})),suggestions:ranked.slice(0,7).map(x=>compact(x.d)),suggestedQueries,categories:CATEGORIES}}
