...
const docs = ROWS.trim().split('\n').map((line, order) => {
  ...
  const summary = `${title} 문법은 ${category} 영역에서 자주 쓰는 Phaser 핵심 문법입니다.`;
  const tags = tagsText.split(' ');
  return { slug, title, category, tags, summary, order, searchText: norm(`${title} ${slug} ${category} ${tagsText} ${summary}`), titleText: norm(`${title} ${slug}`) };
});
function json(data) {
...
export async function onRequestGet({ request }) {
  ...
  const list = docs.map(d => {
    let score = terms.length ? 0 : 1000 - d.order;
    for (const t of terms) {
      if (d.titleText.includes(t)) score += 320;
      if (d.searchText.includes(t)) score += 60;
      if (norm(d.category).includes(t)) score += 40;
    }
    return { d, score };
  })
    .filter(x => x.score > 0)
    .sort((a, b) => b.score - a.score || a.d.order - b.d.order)
    .slice(0, 12)
    .map(x => ({ slug: x.d.slug, title: x.d.title, category: x.d.category, summary: x.d.summary, tags: x.d.tags, snippet: x.d.summary, score: Math.round(x.score) }));
  return json({ query: q, results: list, suggestions: list.slice(0, 8) });
}
