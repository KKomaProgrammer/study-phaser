...
async function loadFullDoc(slug) {
  const direct = await fetchJson('/api/docs?slug=' + encodeURIComponent(slug)).catch(() => null);
  if (direct?.doc) return { doc: direct.doc, related: Array.isArray(direct.related) ? direct.related : [], docs: [] };

  const list = await fetchJson('/api/docs').catch(() => null);
  const docs = Array.isArray(list?.docs) ? list.docs : [];
  const found = findDoc(docs, slug);
  if (!found) return { doc: null, related: [], docs };

  const retry = await fetchJson('/api/docs?slug=' + encodeURIComponent(found.slug)).catch(() => null);
  if (retry?.doc) return { doc: retry.doc, related: Array.isArray(retry.related) ? retry.related : [], docs };
  return { doc: found, related: docs.filter(item => item.slug !== found.slug && item.category === found.category).slice(0, 5), docs };
}

function renderRecoveredDoc(doc, related, docs) {
  const app = document.getElementById('app');
  if (!app || !doc) return;
  ...
  app.innerHTML = `<div class="view doc-view view-ready"><header class="top-nav"><div class="top-nav-inner"><a class="brand-link" href="/" data-nav><span class="brand-orb small-orb">P</span><span>Phaser 문법 검색</span></a><div class="nav-extra"></div><a class="icon-home" href="/" data-nav title="메인으로 돌아가기">⌂</a></div></header><div class="page-layout">${sidebar}<main class="page-content"><main class="doc-shell"><article class="doc-main"><section class="doc-hero ai-surface"><h1>${docFixEsc(doc.title)}</h1><p>${docFixEsc(doc.summary || '')}</p><div class="doc-tags"><span class="badge">${docFixEsc(doc.category || '')}</span>${(doc.tags || []).map(tag => `<span class="tiny-tag">${docFixEsc(tag)}</span>`).join('')}</div></section><section class="doc-body">${sections}<section class="code-lab"><div class="code-lab-head"><div><h2>예제 실행</h2><p>문서 로드 복구 화면입니다. 새로고침 후에도 문서를 표시합니다.</p></div><span class="local-badge">RECOVERED</span></div><div id="ed" class="editor-frame"><textarea class="editor-fallback">${docFixEsc(doc.code || '')}</textarea></div></section></section></article><aside class="related-sidebar"><div class="related-card"><h2>연관된 문서</h2><div class="related-list">${relatedHtml}</div></div></aside></main></main></div></div>`;
}

async function recoverDocumentLoad() {
  const slug = currentDocSlug();
  if (!slug || !isBrokenDocumentScreen()) return;
  const result = await loadFullDoc(slug).catch(() => null);
  if (result?.doc) renderRecoveredDoc(result.doc, result.related, result.docs || []);
}

setTimeout(recoverDocumentLoad, 200);
setTimeout(recoverDocumentLoad, 700);
setTimeout(recoverDocumentLoad, 1600);
setTimeout(recoverDocumentLoad, 3200);
window.addEventListener('popstate', () => setTimeout(recoverDocumentLoad, 250));
document.addEventListener('click', event => {
  if (event.target.closest('[data-nav]')) setTimeout(recoverDocumentLoad, 500);
});
